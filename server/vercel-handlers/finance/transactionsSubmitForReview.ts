...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await executeWithIdempotency(db, context.repository.getIdempotencyRef(), keyHash, payloadHash, async (t) => {
      ...
      const isResubmission = !!((txData as any).returnedToDraftAt || (txData as any).returnedToDraftReason);
      const eventType = isResubmission ? 'resubmitted_for_review' : 'submitted_for_review';
      ...
      t.set(context.repository.getAuditRef().doc(auditId), sanitizeFirestoreObject({
        ...
        action: isResubmission ? 'transaction.resubmitted' : 'transaction.submitted',
        requestId,
        idempotencyKey,
