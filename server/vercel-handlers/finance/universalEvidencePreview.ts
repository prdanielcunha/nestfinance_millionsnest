...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (
      ...
      return res.status(409).json({ error: 'EVIDENCE_PREVIEW_NOT_READY', requestId });
    }
    ...
    if (message === 'FINANCE_ENTITY_NOT_ACTIVE' || message === 'EVIDENCE_PREVIEW_NOT_READY') {
    ...
    if (message === 'EVIDENCE_UNSUPPORTED') {
    ...
    console.error('Universal Evidence Preview Error:', error);
