...
import {
  ...
  UNIVERSAL_EVIDENCE_NATIVE_TEXT_MAX_BYTES,
} from './universalEvidencePdfNativeText.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (byteSize > UNIVERSAL_EVIDENCE_NATIVE_TEXT_MAX_BYTES) {
      return res.status(413).json({ error: 'EVIDENCE_NATIVE_TEXT_TOO_LARGE', requestId });
    }
    ...
    const structure = inspectPdfStructure(stored.bytes);
    if (
      structure.encrypted ||
      structure.textLayerState !== 'detected' ||
      structure.unsupportedStreams !== 0 ||
      structure.limited
    ) {
      return res.status(409).json({ error: 'EVIDENCE_NATIVE_TEXT_UNAVAILABLE', requestId });
    }

    let extraction;
    try {
      extraction = await extractNativePdfText(stored.bytes);
    } catch (error: any) {
      if (String(error?.message || '') === 'EVIDENCE_NATIVE_TEXT_TOO_LARGE') {
        return res.status(413).json({ error: 'EVIDENCE_NATIVE_TEXT_TOO_LARGE', requestId });
      }
      return res.status(409).json({ error: 'EVIDENCE_NATIVE_TEXT_UNAVAILABLE', requestId });
    }
    ...
    if (
      message === 'FINANCE_ENTITY_NOT_ACTIVE' ||
      message === 'EVIDENCE_ANALYSIS_NOT_READY' ||
      message === 'EVIDENCE_NATIVE_TEXT_UNAVAILABLE'
    ) {
      return res.status(409).json({ error: message, requestId });
    }
    if (message === 'EVIDENCE_TOO_LARGE' || message === 'EVIDENCE_NATIVE_TEXT_TOO_LARGE') {
    ...
    console.error('Universal Evidence PDF Text Error:', {
      requestId,
      name: error?.name || 'Error',
      message: String(error?.message || 'INTERNAL_SERVER_ERROR').slice(0, 160),
    });
