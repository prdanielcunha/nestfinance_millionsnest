...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { direction, amountCents, occurredAt, accountId, paymentMethod, allocations, description, sourceContext } = payload;
    
    if (direction !== 'income' && direction !== 'expense') {
      return res.status(400).json({ error: 'INVALID_PARAMETERS', details: 'Direction must be income or expense' });
    }
