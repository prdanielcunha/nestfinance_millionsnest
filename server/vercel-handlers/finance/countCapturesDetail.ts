    ...
    if (capture.status === 'duplicate') {
      return res.status(200).json({
        capture: {
          ...
          appliedToCount: capture.appliedToCount ? {
            countSessionId: String(capture.appliedToCount.countSessionId || ''),
            stage: capture.appliedToCount.stage,
            sessionVersion: Number(capture.appliedToCount.sessionVersion || 0),
            resultingStatus: String(capture.appliedToCount.resultingStatus || ''),
            appliedAt: toOptionalIso(capture.appliedToCount.appliedAt),
          } : null,
          originalUrl: null,
          normalizedUrl: null,
