...
export default function UniversalCapturePage() {
  const { language } = useLanguage(); const copy = UNIVERSAL_CAPTURE_COPY[language];
  const { activeFinanceEntityId } = useFinanceEntity(); const { accessState } = useAuth();
  const organizationId = accessState.organizationId || accessState.organization?.id || '';
  const inputRef = useRef<HTMLInputElement>(null); const sourceRef = useRef<UniversalEvidenceSourceKind>('file');
  const keysRef = useRef<{ fingerprint: string; start: string; finalize: string } | null>(null);
  const [state, setState] = useState<State>('selecting'); const [file, setFile] = useState<File | null>(null); const [preview, setPreview] = useState<string | null>(null); const [notice, setNotice] = useState<string | null>(null);
  const clipboardAvailable = typeof navigator !== 'undefined' && Boolean(navigator.clipboard?.read);
  useEffect(() => () => { if (preview) URL.revokeObjectURL(preview); }, [preview]);
  const reset = () => { if (preview) URL.revokeObjectURL(preview); setPreview(null); setFile(null); setNotice(null); setState('selecting'); keysRef.current = null; };
  const adopt = (next: File | null, source: UniversalEvidenceSourceKind) => {
    setNotice(null); if (!next || next.size <= 0) { setState('corrupt'); return; }
    if (!isUniversalEvidenceMime(next.type)) { setState('unsupported'); return; }
    if (next.size > UNIVERSAL_EVIDENCE_MAX_BYTES) { setState('too_large'); return; }
    if (preview) URL.revokeObjectURL(preview); setFile(next); sourceRef.current = source; setPreview(next.type.startsWith('image/') ? URL.createObjectURL(next) : null); setState('preview'); keysRef.current = null;
  };
  const choose = (source: UniversalEvidenceSourceKind) => { sourceRef.current = source; if (source === 'camera') inputRef.current?.setAttribute('capture', 'environment'); else inputRef.current?.removeAttribute('capture'); inputRef.current?.click(); };
  const paste = async () => {
    try { const items = await navigator.clipboard.read(); for (const item of items) { const type = item.types.find(isUniversalEvidenceMime); if (type) { const blob = await item.getType(type); adopt(new File([blob], `clipboard-${Date.now()}.${type.split('/')[1]}`, { type }), 'clipboard'); return; } } setNotice(copy.clipboardEmpty); }
    catch { setNotice(copy.clipboardEmpty); }
  };
  const submit = async () => {
    if (!file || !activeFinanceEntityId || !organizationId) return;
    setState('validating'); const fingerprint = `${file.name}:${file.size}:${file.lastModified}`;
    if (!keysRef.current || keysRef.current.fingerprint !== fingerprint) keysRef.current = { fingerprint, start: universalCaptureService.token('idevidence_start'), finalize: universalCaptureService.token('idevidence_finalize') };
    try { const result = await universalCaptureService.accept(organizationId, activeFinanceEntityId, file, sourceRef.current, keysRef.current); setState(result.duplicate ? 'duplicate' : 'accepted'); }
    catch (error: any) { const code = String(error?.code || ''); setState(code.includes('UNSUPPORTED') ? 'unsupported' : code.includes('TOO_LARGE') ? 'too_large' : code.includes('CORRUPT') ? 'corrupt' : 'recoverable_error'); }
  };
  ...
  return <div className="mx-auto flex min-h-[65vh] max-w-2xl flex-col justify-center py-6 sm:py-10">
    ...
    {activeFinanceEntityId && (state === 'preview' || state === 'validating') && file ? <Surface variant="elevated" radius="xl" className="overflow-hidden">
      ...
      <div className="p-5 sm:p-6"><h2 className="text-lg font-semibold">{copy.preview}</h2><p className="mt-1 truncate text-sm text-text-secondary">{file.name} · {(file.size / 1024 / 1024).toFixed(1)} MB</p><div className="mt-5 flex flex-col gap-3 sm:flex-row"><Button size="lg" onClick={submit} disabled={state === 'validating'}><Check className="h-5 w-5" aria-hidden="true" />{state === 'validating' ? copy.validating : copy.validate}</Button><Button size="lg" variant="ghost" onClick={reset} disabled={state === 'validating'}>{copy.change}</Button></div></div>
    </Surface> : null}
    {activeFinanceEntityId && terminal ? <Surface variant="elevated" radius="xl" className="p-7 text-center sm:p-10" role="status" aria-live="polite"><div className={`mx-auto flex h-14 w-14 items-center justify-center rounded-2xl ${state === 'accepted' || state === 'duplicate' ? 'bg-semantic-success/10 text-semantic-success' : 'bg-semantic-danger/10 text-semantic-danger'}`}>{state === 'accepted' || state === 'duplicate' ? <Check className="h-7 w-7" /> : <RefreshCw className="h-7 w-7" />}</div><h2 className="mt-5 text-xl font-semibold">{copy[state]}</h2><Button className="mt-6" size="lg" variant="secondary" onClick={reset}>{copy.again}</Button></Surface> : null}
