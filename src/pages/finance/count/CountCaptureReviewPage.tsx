...
function EvidenceRegionCrop({ url, region, label, width, height }: { url: string; region: CountCaptureRegion; label: string; width: number; height: number }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border-subtle bg-white" style={{ aspectRatio }} role="img" aria-label={label}>
      <div className="relative h-full w-full overflow-hidden">
        <img
          ...
          draggable={false}
          style={{ width: `${100 / region.width}%`, height: `${100 / region.height}%`, left: `${-(region.x / region.width) * 100}%`, top: `${-(region.y / region.height) * 100}%` }}
        />
      </div>
    </div>
  );
...
function CountCaptureReviewContent() {
  ...
  useEffect(() => {
    setCapture(null); saveAttemptRef.current = null; void load();
  ...
  const preparedFields = useMemo(() => {
    ...
    for (const key of COUNT_CAPTURE_FIELD_KEYS) {
      ...
      if (row.unreadable) { result.push({ key, decision: 'unreadable', valueCents: null }); continue; }
  ...
  const regionFor = (key: CountCaptureFieldKey): CountCaptureRegion | null => {
    ...
    const candidate = capture.candidates?.find((field) => field.key === key);
    return candidate?.region || getCountCaptureEvidenceRegion(capture.templateVersion || 1, key);
  };
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-surface-base pb-24 md:pb-8">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6 lg:px-8"><div className="mx-auto flex w-full max-w-6xl flex-col gap-5">
