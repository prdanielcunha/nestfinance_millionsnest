...
function TransactionEditContent() {
  ...
  const { getTransactionDetail, updateDraft, submitForReview } =
    useTransactions();
  ...
  const [txExpectedVersion, setTxExpectedVersion] = useState<number | null>(
    null,
  );
  ...
  const loadDataAndCatalogs = async (
    ...
    try {
      ...
      if (
        txData.accountId &&
        !activeAccounts.some((a) => a.id === txData.accountId)
      ) {
        activeAccounts.push({
          id: txData.accountId,
          name: txData.accountName || "Conta inativa",
          active: false,
        });
      }
      for (const a of txAllocs) {
        if (a.categoryId && !activeCats.some((c) => c.id === a.categoryId)) {
          activeCats.push({
            id: a.categoryId,
            name: a.categoryName || "Categoria inativa",
            active: false,
            kind: txData.direction,
          });
        }
        if (a.fundId && !activeFunds.some((f) => f.id === a.fundId)) {
          activeFunds.push({
            id: a.fundId,
            name: a.fundName || "Fundo inativo",
            active: false,
          });
        }
      }
      ...
      if (
        txAllocs.length > 1 ||
        (txAllocs.length === 1 &&
          txAllocs[0].amountCents !== txData.amountCents)
      ) {
        setIsSplit(true);
        setAllocations(
          txAllocs.map((a: any) => ({
        ...
        setAllocations([
          {
            ...
            costCenterId: txAllocs[0].costCenterId || "",
        ...
        setIsSplit(false);
        setAllocations([
          { id: "alloc_initial", categoryId: "", fundId: "", amountRaw: null },
        ]);
      }
    } catch (err: any) {
      if (signal?.aborted || currentEpoch !== epochRef.current) return;
  ...
  const availableAccounts = useMemo(() => {
    return getCompatibleAccounts(
      paymentMethod,
      direction,
      accounts.filter((a) => a.active || a.id === accountId),
    );
  ...
  const handleErrorContext = (err: any) => {
    ...
    if (msg.includes("FINANCE_ALLOCATION_TOTAL_MISMATCH"))
      ...
      msg =
        "Não foi possível confirmar se o rascunho foi salvo. Tente novamente com segurança.";
    }
      ...
      try {
        ...
        setTimeout(() => {
          if (mode === "draft") {
            ...
            handleSaveAndSubmitWithRetryFlag(true);
          }
        }, 50);
  ...
  const handleSaveDraft = async () => {
    await handleSaveDraftWithRetryFlag(false);
  };
  ...
  const handleSaveAndSubmit = async () => {
    await handleSaveAndSubmitWithRetryFlag(false);
  };

  const handleSaveAndSubmitWithRetryFlag = async (isRetry: boolean) => {
    if ((saving || submitting) && !isRetry) return;
    setSaveError(null);
    ...
    if (direction !== "transfer") {
      if (!paymentMethod) {
        setSaveError(
          "Para concluir, informe a forma de pagamento e a categoria.",
        );
        return;
      }
      ...
      if (totalCents !== calculatedAllocated) {
        setSaveError(
          "A soma dos rateios deve ser exatamente igual ao valor total para concluir.",
        );
        return;
      }
    }
  ...
  if (immutableStatusError) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-surface-base border-t border-border-subtle">
        ...
        <p className="text-sm text-text-muted max-w-sm mb-6">
          Esta movimentação não pode mais ser editada.
        </p>
        <button
          onClick={() =>
            navigate(
  ...
  const handleBeforeSwitch = () => {
    if (!saveSuccess) {
      const confirmDiscard = window.confirm(
        "Você tem alterações que podem não estar salvas. Deseja sair e trocar de igreja? As alterações não salvas serão descartadas.",
      );
              ...
              {conflictError && (
                <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl flex flex-col gap-3 text-sm items-start">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full mt-2">
                    <button
                      onClick={() =>
                        loadDataAndCatalogs(undefined, epochRef.current)
                      }
                        ...
                        <AccountRepairCard
                          ...
                          onRepaired={(repairedAcc) => {
                            ...
                            if (pendingSubmitRef.current === "draft") {
                              ...
                              pendingSubmitRef.current = null;
                              setTimeout(() => handleSaveAndSubmitWithRetryFlag(true), 50);
                            }
                          }}
                        />
                      )}
                   ...
                   <div className="flex flex-col gap-4 mt-2 p-5 bg-surface-secondary/30 rounded-2xl border border-border-subtle/50">
                     <div className="flex flex-col gap-1.5 focus-within:relative focus-within:z-10">
                       ...
                       <input
                         type="text"
                         value={counterparty}
                         onChange={(e) => { setCounterparty(e.target.value); setSaveSuccess(null); }}
                     ...
                     <div className="flex flex-col gap-1.5 focus-within:relative focus-within:z-10 mt-4">
                       ...
                       <textarea
                         ...
                         onChange={(e) => { setEvidenceJustification(e.target.value); setSaveSuccess(null); }}
                  ...
                  <div className="sticky bottom-0 bg-surface-base/95 backdrop-blur-md border-t border-border-subtle p-4 pb-[calc(1rem+env(safe-area-inset-bottom,16px))] -mx-4 mt-8 sm:-mx-6 lg:-mx-8 md:static md:bg-transparent md:border-t-0 md:p-0 md:backdrop-blur-none md:mt-4 z-20 flex flex-col gap-3">
                    {hasEffectiveCapability(
                      ...
                      <button
                        ...
                        disabled={saving || conflictError || !readiness.ready}
