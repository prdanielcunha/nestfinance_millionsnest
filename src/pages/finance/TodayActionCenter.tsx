...
const COPY: Record<Language, TodayCopy> = {
  PT: {
    ...
    approvedTitle: (count) => `${count} ${count === 1 ? 'movimentação foi conferida e aguarda lançamento' : 'movimentações foram conferidas e aguardam lançamento'}`,
    approvedText: 'Os dados foram conferidos. Eles ainda não entraram no saldo contábil; o lançamento real continua bloqueado nesta fase.',
    ...
    statusReview: 'Aguardando conferência',
    statusApproved: 'Conferida · aguardando lançamento',
  ...
  EN: {
    ...
    approvedTitle: (count) => `${count} ${count === 1 ? 'transaction was checked and is waiting for posting' : 'transactions were checked and are waiting for posting'}`,
    approvedText: 'The information was checked. It has not entered the accounting balance; real posting remains disabled in this phase.',
    ...
    statusApproved: 'Checked · waiting for posting',
  ...
  ES: {
    ...
    approvedTitle: (count) => `${count} ${count === 1 ? 'movimiento fue revisado y espera registro' : 'movimientos fueron revisados y esperan registro'}`,
    approvedText: 'Los datos fueron revisados. Todavía no entraron en el saldo contable; el registro real continúa bloqueado en esta fase.',
    ...
    statusReview: 'Esperando revisión',
    statusApproved: 'Revisado · esperando registro',
...
export function TodayActionCenter() {
  ...
  const summaryItems = useMemo(() => {
    ...
    const approved = {
      ...
      emphasis: effectiveSummary.approvedForPosting > 0 ? 'text-semantic-warning' : 'text-text-primary',
    };
    const drafts = {
      id: 'drafts',
      label: copy.drafts,
