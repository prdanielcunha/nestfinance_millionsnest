...
- o histórico relacionado é carregado somente após ação explícita e reutiliza `audit-list`, a trilha canônica server-mediated da entidade; o cliente filtra somente a movimentação atual e os comprovantes já vinculados, limita a exibição e nunca cria/edita/apaga auditoria;
- quando `audit-list.truncated === true`, a UI informa que o painel representa apenas a janela recente e mantém a Auditoria completa como destino canônico;
