...
check(!handler.includes('original.path') || !handler.includes('json({ path'), 'preview never returns the private Storage path as JSON');
...
check(copy.includes("previewOriginal: 'Visualizar original'") && copy.includes("previewOriginal: 'View original'") && copy.includes("previewOriginal: 'Visualizar original'"), 'preview action is localized in PT/EN/ES');
