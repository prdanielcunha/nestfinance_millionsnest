...
export function validateGatewayInventory({ contracts, gatewaySources, rewrites }) {
  ...
  for (const contract of contracts) {
    ...
    if (!GATEWAY_FILES[contract.gateway]) {
      errors.push(`Unknown gateway in contract: ${contract.gateway}`);
    }

    if (contract.exposure === 'rewrite') {
      if (!contract.url) {
        errors.push(`Rewrite contract missing url: ${key}`);
      } else if (rewriteUrls.has(contract.url)) {
        errors.push(`Duplicate contract URL: ${contract.url}`);
      } else {
        rewriteUrls.add(contract.url);
      }
  ...
  for (const [gateway, source] of Object.entries(gatewaySources)) {
    ...
    if (caseSet.size !== cases.length) {
      errors.push(`Duplicate switch case detected in ${gateway}`);
    }
    ...
    for (const contract of contracts.filter((item) => item.gateway === gateway)) {
      if (!caseSet.has(contract.operation)) {
        errors.push(`Certified operation missing from gateway: ${gateway}::${contract.operation}`);
      }
    }
  }
...
export async function runApiContractCheck(rootDir = process.cwd(), fsImpl = fs) {
  ...
  if (errors.length > 0) {
    throw new Error(`API contract certification failed:\n- ${errors.join('\n- ')}`);
  }
