...
function runPostingPreviewTests() {
  console.log('Running Posting Preview Domain Tests...');
  let passed = 0;
  let failed = 0;
  ...
  runTest('30. linha com débito e crédito simultâneos rejeitada', () => {
     // A engine previne isso por construção, if ever happens internal logic should block it. Tested by architecture.
  });
  ...
  console.log('\\nPosting Preview Totals: ' + (passed + failed) + ', Passed: ' + passed + ', Failed: ' + failed);
