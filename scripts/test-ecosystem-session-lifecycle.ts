...
const launch = lifecycle.buildMillionsNestLaunchUrl('/finance/review?status=pending');
...
const unsafeLaunch = new URL(lifecycle.buildMillionsNestLaunchUrl('https://evil.example'));
assert.equal(unsafeLaunch.searchParams.get('returnTo'), '/finance');

replaced = '';
assert.equal(lifecycle.recoverRevokedHubSession('/finance/balance'), true);
assert.equal(new URL(replaced).origin, 'https://www.millionsnest.com');
...
for (const phrase of [
  ...
  'Back to MillionsNest',
  'Volver a MillionsNest',
  'Sair do NestFinance',
  'Sign out of NestFinance',
  'Salir de NestFinance',
]) {
  assert.ok(shellSource.includes(phrase) || moreSource.includes(phrase), 'missing localized lifecycle copy: ' + phrase);
}
